#!/usr/bin/python
# coding: UTF-8
import io.StringIO, pycurl, sys, os, time
...
def test_gzip(input_url):
    ...
    print
    'DNS解析时间：%.2f ms' % (dns_resolve * 1000)
    print
    '建立连接时间： %.2f ms' % (http_conn_time * 1000)
    print
    '准备传输时间： %.2f ms' % (http_pre_trans * 1000)
    print
    "传输开始时间： %.2f ms" % (http_start_trans * 1000)
    print
    "传输结束时间： %.2f ms" % (http_total_time * 1000)
    print
